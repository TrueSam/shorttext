...
function DataSetTest.BatchTest()
  ...
  assert(dataset.indices_:size(1) == config.kSampleSize)
  dataset.indices_, _ = torch.sort(dataset.indices_, 1)
  ...
  assert(dataset.lengths_[1] == 12)
  assert(dataset.lengths_[2] == 11)
  assert(dataset.lengths_[3] == 12)
  assert(dataset.lengths_[4] == 12)
  assert(dataset.lengths_[5] == 12)
  assert(dataset.lengths_[6] == 7)
  assert(dataset.lengths_[7] == 2)

  assert(dataset.starts_[1] == 1)
  assert(dataset.starts_[2] == 13)
  assert(dataset.starts_[3] == 24)
  assert(dataset.starts_[4] == 36)
  assert(dataset.starts_[5] == 48)
  assert(dataset.starts_[6] == 60)
  assert(dataset.starts_[7] == 67)



  for s = 1, 100 do
    local start = dataset.current_
  end
  local batch, label = dataset:SampleBatch()
  assert(#batch == 3)  -- prev, curr, next
  for k = 1, #batch do
    assert(#batch[k], config.kSentenceSize)
    for i = 1, #batch[k] do
